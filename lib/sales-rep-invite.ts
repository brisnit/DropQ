...
export async function sendSalesRepInvite(repId: string): Promise<InviteResult> {
  ...
  const base = baseUrl();
  const signupUrl = `${base}/vendor/signup?ref=${rep.referralCode}`;
  ...
  if (rep.phone) {
    ...
    try {
      await sendSms(
        rep.phone,
        `You've been invited to sell with DropQ. Your referral code is ${rep.referralCode}. ` +
          `Share this vendor signup link: ${signupUrl}. Create your account with ${rep.email} to track referrals.`
      );
